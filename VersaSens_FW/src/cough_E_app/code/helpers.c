...
void argsort(uint16_t *arr, uint16_t len, uint16_t *sort_idxs){
    ...
    argsort_struct *elems = (argsort_struct*)k_malloc(len * sizeof(argsort_struct));
...
void order_by_idxs(void *arr_in, uint16_t len, uint16_t *idxs, type_sort_t type){
    ...
    if(type == FLOAT_SORT){
        ...
        uint16_t *tmp = (uint16_t*)k_malloc(len * sizeof(uint16_t));
...
void padding(const float *sig, int len, int padlen, float *res){
    ...
    float *left_ext = (float*)k_malloc(padlen * sizeof(float));
    float *right_ext = (float*)k_malloc(padlen * sizeof(float));
