...
void stft(const float *x, int16_t len, int16_t n_frames, float *res){
    ...
    float *padded = (float*)k_malloc(padded_len * sizeof(float));
    // zero_padding(x, len, PAD_LEN, padded);
    reflect_padding(x, len, PAD_LEN, padded);

    float *column = (float*)k_malloc(N_FFT * sizeof(float));
    float *fft_res = (float*)k_malloc(FFT_RES_LEN * sizeof(float));

    // initialize RFFT structures
    kiss_fftr_cfg cfg = kiss_fftr_alloc(N_FFT, 0, 0, 0);
    kiss_fft_cpx *cx_out = (kiss_fft_cpx*)k_malloc(N_FFT *sizeof(kiss_fft_cpx));
...
void mel_spectrogram_full(const float *x, int16_t len, int16_t n_frames, float *res){
    ...
    // STFT
    float *frames_power = (float*)k_malloc((FFT_RES_LEN * n_frames) * sizeof(float));
...
void dct_matrix(float *x, int16_t rows, int16_t cols, float *y){
    ...
    float *column = (float*)k_malloc(rows * sizeof(float));
    float *c_res = (float*)k_malloc(rows * sizeof(float));
