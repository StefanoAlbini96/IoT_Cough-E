...
void compute_rfft(const float *sig, int16_t len, int16_t fs, float *mags, float *freqs, float *sum_mags){
    ...
    float *re = (float*)k_malloc(len * sizeof(float));
    float *im = (float*)k_malloc(len * sizeof(float));
...
void _rfft(const float *sig, int16_t len, float *real, float *imag){
    ...
    kiss_fft_cpx *cx_out = (kiss_fft_cpx *) k_malloc(len * sizeof(kiss_fft_cpx));
...
void compute_periodogram(const float *sig, int16_t len, int16_t fs, float *psd, float *freqs){
    ...
    float *win = (float*)k_malloc(NPERSEG * sizeof(float));   // to keep the data of the current processed window
    float *cumul_sums = (float*)k_malloc(NPERSEG * sizeof(float));  // To store the cumulative sum of the FFT of each frequency bin
    memset(cumul_sums, 0, NPERSEG * sizeof(float));

    // To store the real and imaginary parts of each FFT
    float *re = (float*)k_malloc(NPERSEG * sizeof(float));
    float *im = (float*)k_malloc(NPERSEG * sizeof(float));

    // To store the magnitudes squared after the FFT
    float *mags_squared = (float*)k_malloc(((NPERSEG/2)+1) * sizeof(float));
...
float compute_spectral_entropy(float *x, int16_t len){
    ...
    float *tmp = (float*)k_malloc(len * sizeof(float));
...
void mfcc_computation(const float *x, int16_t len, int16_t n_frames, float *coeffs){
    ...
    float *db_power = (float*)k_malloc((MEL_ROWS * n_frames) * sizeof(float));
...
void get_mfcc_features(const float *x, int16_t len, float *mean_mfcc, float *std_mfcc){
    ...
    float *coeffs = (float*)k_malloc((N_MFCC*n_frames) * sizeof(float));
...
//     float *spectrogram = (float*)k_malloc((MEL_ROWS * n_frames) * sizeof(float));
...
//     float *mel_dB = (float*)k_malloc((MEL_ROWS * n_frames) * sizeof(float));
...
void get_mel_spectrogram_features(const float *x, int16_t len, uint8_t *idx_needed, uint8_t n_mels_needed, float *mean_mel_spectr, float *std_mel_spectr, float *max_mel_spectr, float *entropy_mel_spectr){
    ...
    float *spectrogram = (float*)k_malloc((n_mels_needed * n_frames) * sizeof(float));
    ...
    // Stores the dB of the mel spectrogram
    float *mel_dB = (float*)k_malloc((n_mels_needed * n_frames) * sizeof(float));
