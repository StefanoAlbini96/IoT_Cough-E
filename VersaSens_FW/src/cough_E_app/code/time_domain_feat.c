...
void eepd(const float *sig, int16_t len, int16_t fs, const int8_t *select, int16_t *res){
    ...
    float *interm = (float*)k_malloc(len * sizeof(float));    // to store the intermediate result between the first and the second filter 
    float *filtered = (float*)k_malloc(len * sizeof(float));       // temporary to store the result of each filter
