...
void filtfilt(const float *sig, int len, const float *b, const float* a, const float *zi, float *res){
    ...
    float *pad = (float*)k_malloc(padded_len * sizeof(float));
    ...
    // FILTER FORWARD //
    float *intermediate = (float*)k_malloc(padded_len * sizeof(float)); // intermediate output, cannot be res because it's not padded

    float *initial = (float*)k_malloc(2 * sizeof(float));
    initial[0] = zi[0] * x0;
    initial[1] = zi[1] * x0;
    linear_filer(pad, padded_len, b, a, initial, intermediate);

    // FILTER BACKWARD //
    float *reverse = (float*)k_malloc(padded_len * sizeof(float));    // for the reverse filtering
    ...
    float *res_padded = (float*)k_malloc(padded_len * sizeof(float));
